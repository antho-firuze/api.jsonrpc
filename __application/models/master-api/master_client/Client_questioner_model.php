...
class Client_questioner_model extends CI_Model
{
	...
	private function _sql($request)
	{
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('T1.simpiID, T1.ClientID, T1.SalesID, T2.SalesCode, T2.TreePrefix, T2.SInvestCode, 
							T3.QuestionerDate, T3.RiskID, T4.RiskCode, T3.RiskValue, T3.IsLast');  
		$this->db->from('master_client T1');
		$this->db->join('master_sales T2', 'T1.simpiID = T2.simpiID And T1.SalesID = T2.SalesID');  
		$this->db->join('master_client_questioner T3', 'T1.simpiID = T3.simpiID And T1.ClientID = T3.ClientID');  
		$this->db->join('parameter_client_risklevel T4', 'T3.RiskID = T4.RiskID');  
	...
	function load($request)
	{
		...
		$this->_sql($request);
		$this->db->where('T3.QuestionerDate', $request->params->QuestionerDate);
	...
	{	
		...
		$this->load->database($return);
		$this->_sql($request);
		if (isset($request->params->IsLast) && !empty($request->params->IsLast)) 
			$this->db->where('T3.IsLast', $request->params->IsLast);	
		if (isset($request->params->ClientID) && !empty($request->params->ClientID)) {
