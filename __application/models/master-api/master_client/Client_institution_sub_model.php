...
class Client_institution_sub_model extends CI_Model
{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
		   $this->db->select($request->params->fields);
			...
			$this->db->select('T1.simpiID, T1.ClientID, T1.ClientCode, T1.SID, T1.IFUA, T1.ReferralCode, T1.ClientName, 
					 ...
					 T3.ParentID, T4.ClientCode As ParentCode, T4.ClientName As ParentName, T3.SubSeparator, T3.SubName,  
					 T3.IsSeparateAccount, T5.BusinessActivityID, T5.BusinessTypeID, T5.TaxID, T5.SignatureRule
        			 T5.MMN, T5.TaxID, T5.Gender, T5.NationalityID, T5.LevelID, T5.ReligionID, T5.OccupationID, T5.OfficeName, 
        			 T5.OfficeAddress, T5.OfficePhone, T5.OfficeBusinessActivityID, T5.MaritalStatusID, T5.SpouseName, 
        			 T5.SpouseBirthDate, T5.IDCardTypeID, T5.IDCardNo, T5.IDCardIssuer, T5.IDCardIsExpired, T5.IDCardExpired');  
		$this->db->from('master_client T1');
		$this->db->join('master_sales T2', 'T1.simpiID = T2.simpiID And T1.SalesID = T2.SalesID');  
		$this->db->join('master_client_institution_sub T3', 'T1.simpiID = T2.simpiID And T1.ClientID = T3.ClientID');  
		$this->db->join('master_client T4', 'T3.simpiID = T4.simpiID And T3.ParentID = T4.ClientID');  
		$this->db->join('master_client_institution T5', 'T4.simpiID = T5.simpiID And T4.ClientID = T5.ClientID');  
	...
	function search($request)
	{
		...
		} elseif (isset($request->params->ClientCode)) {
		...
		} elseif (isset($request->params->IFUA)) {
