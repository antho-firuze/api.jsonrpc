...
class Client_document_model extends CI_Model
{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
		   $this->db->select($request->params->fields);
            ...
            $this->db->select('T1.simpiID, T1.ClientID, T1.SalesID, T2.SalesCode, T2.TreePrefix, T2.SInvestCode,  
								T3.TypeID, T3.DocumentIssuer, T3.DocumentNo, T3.DocumentTitle, T3.DocumentIsExpired, T3.DocumentExpired');  
		$this->db->from('master_client T1');
		$this->db->join('master_sales T2', 'T1.simpiID = T2.simpiID And T1.SalesID = T2.SalesID');  
		$this->db->join('master_client_institution_document T3', 'T1.simpiID = T3.simpiID And T1.ClientID = T3.ClientID');  
	...
	function load($request)
	{
		...
		if (!isset($request->params->DocumentTypeID) || empty($request->params->DocumentTypeID)) {
			$return = $this->system->error_data('00-1', $request->LanguageID, 'parameter DocumentTypeID');
			return [FALSE, ['message' => $return['message'], 'error' => $return['error']]];		 
		}
		...
		$this->_sql($request);
		$this->db->where('T3.TypeID', $request->params->DocumentTypeID);
