...
class Detail_model extends CI_Model
{
	function __construct() {
		...
		function detail_search($request)
		{	
			...
			if (isset($request->params->fields) && !empty($request->params->fields))
				...
				$this->db->select('PortfolioID, SecuritiesID, PositionDate, SourceID, 
														AcqNo, AcqDate, AcqSettle, AcqQty, AcqPrice, AcqCost');
			$this->db->from('afa_securities_balance_detail');
		...
		function detail_history($request)
		{
			//cek akses: by 4 method
			list($success, $return) = $this->system->is_valid_access4($request);
			if (!$success) return [FALSE, $return];
		...
		{	
			...
			if (isset($request->params->fields) && !empty($request->params->fields))
				...
				$this->db->select('simpiID, PortfolioID, SecuritiesID, PositionDate, SourceID, AcqNo, FeeID, FeeAmount');
			$this->db->from('afa_securities_balance_detail_charges');
			$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
			$this->db->where('PositionDate', $request->params->PositionDate, NULL, FALSE);
			//daftar unit detail by PortfolioID: 1 atau lebih
			if (isset($request->params->PortfolioID)) {
