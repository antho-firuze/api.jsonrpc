	...
	function __construct() {
		...
		{	
			...
			if (isset($request->params->fields) && !empty($request->params->fields))
				...
				$this->db->select('simpiID, PortfolioID, SecuritiesID, PositionDate, DailyUnrealizedPL, DailyRealizedPL, 
									DailyIncome, rDUP, rDRP, rDI, giDUP, giDRP, giDI');
			$this->db->from('afa_income_securities_balance');
		...
		function balance_history($request)
		{
			...
			if (isset($request->params->fields) && !empty($request->params->fields))
				...
				$this->db->select('simpiID, PortfolioID, SecuritiesID, PositionDate, DailyUnrealizedPL, 
														DailyRealizedPL, DailyIncome, rDUP, rDRP, rDI, giDUP, giDRP, giDI');
			$this->db->from('afa_income_securities_balance');
			$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
			//daftar unit referral by PortfolioID: 1 atau lebih
			if (isset($request->params->PortfolioID)) {
		...
		function transaction_search($request)
		{
			...
			if (isset($request->params->fields) && !empty($request->params->fields))
				...
				$this->db->select('simpiID, TrxID, PortfolioID, SecuritiesID, TrxLinkType, TrxLinkID, TrxDate, 
														TrxDescription, TrxDebit, TrxCredit, TrxSourceID, TrxSource2ID');
			$this->db->from('afa_income_securities_transaction');
			$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
