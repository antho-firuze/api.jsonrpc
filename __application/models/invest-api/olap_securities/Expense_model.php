	...
	function __construct() {
		...
		{	
			...
			if (isset($request->params->fields) && !empty($request->params->fields))
				...
				$this->db->select('simpiID, PortfolioID, SecuritiesID, FeeID, PositionDate, DailyExpense, rDE, giDE');
			$this->db->from('afa_expense_securitiescharges_balance');
			...
			if (isset($request->params->SecuritiesID)) {
			 ...
			 //daftar unit referral by FeeID: 1 atau lebih
		...
		function transaction_search($request)
		{
			...
			if (isset($request->params->fields) && !empty($request->params->fields))
				...
				$this->db->select('simpiID, TrxID, PortfolioID, SecuritiesID, FeeID, TrxLinkType, TrxLinkID, 
														TrxDate, TrxDescription, TrxDebit, TrxCredit, TrxSourceID, TrxSource2ID');
			$this->db->from('afa_expense_securitiescharges_transaction');
			$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
