...
class Payable_redemption_model extends CI_Model
{
	...
	{	
		...
		if (isset($request->params->fields) && !empty($request->params->fields))
			...
			$this->db->select('simpiID, PortfolioID, ClientID, PositionDate, PayableBalance');
		$this->db->from('afa_payable_redemption_balance');
	...
	function transaction_search($request)
	{
		...
		$this->db->from('afa_payable_redemption_transaction');
		$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
