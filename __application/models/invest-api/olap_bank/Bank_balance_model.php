...
class Bank_balance_model extends CI_Model
{
	...
	private function _sql($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('simpiID, PortfolioID, CcyID, AccountID, PositionDate, 
								BankID, BankTypeID, TDTermID, AccountBalance, AccountInterest');
		$this->db->from('afa_bankaccount_balance');
	...
	private function _sum($request, $groupSelect = NULL)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$sum_sql .= 'SUM(AccountBalance) As AccountBalance, SUM(AccountInterest) As AccountInterest';
	...
	private function _avg($request, $groupSelect = NULL)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$sum_sql = '';
			if (!is_null($groupSelect)) $sum_sql = $groupSelect.', ';
			$sum_sql .= 'AVG(AccountBalance) As AccountBalance, AVG(AccountInterest) As AccountInterest';
			$this->db->select($sum_sql);
		}
		$this->db->from('afa_bankaccount_balance');
		$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
