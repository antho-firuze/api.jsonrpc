...
class Expensecharges_balance_model extends CI_Model
{
	...
	private function _sql($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('simpiID, PortfolioID, CcyID, FeeID, PositionDate, DailyExpense, rDE, giDE');
		$this->db->from('afa_expense_charges_balance');
	...
	private function _sum($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('SUM(DailyExpense) As DailyExpense');
		$this->db->from('afa_expense_charges_balance');
		$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
	...
	private function _avg($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('AVG(DailyExpense) As DailyExpense');
		$this->db->from('afa_expense_charges_balance');
		$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
