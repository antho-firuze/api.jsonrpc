...
class Assetothers_balance_model extends CI_Model
{
	...
	private function _sql($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('simpiID, PortfolioID, CcyID, AssetID, PositionDate, AssetBalance');
		$this->db->from('afa_assetothers_balance');
	...
	private function _sum($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('SUM(AssetBalance) As AssetBalance');
		$this->db->from('afa_assetothers_balance');
		$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
	...
	private function _avg($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('AVG(AssetBalance) As AssetBalance');
		$this->db->from('afa_assetothers_balance');
		$this->db->where('simpiID', $request->simpi_id, NULL, FALSE);
	...
	function load($request)
	{
		...
		if ((strtolower($request->params->option_aggregate)=='sum') || 
			...
			if (isset($request->params->AssetID) && !empty($request->params->AssetID)) {
				$this->db->where('AssetID', $request->params->AssetID);
			} elseif (isset($request->params->AssetList) && !empty($request->params->AssetList)) {
				if (is_array($request->params->AssetList)) {
					$this->db->where_in('AssetID', $request->params->AssetList);
				} else {
					$return = $this->system->error_data('00-1', $request->LanguageID, 'parameter AssetList');
					return [FALSE, ['message' => $return['message'], 'error' => $return['error']]];		 
				}
			}	
		} else {
			...
			if (!isset($request->params->AssetID) || empty($request->params->AssetID)) {
				$return = $this->system->error_data('00-1', $request->LanguageID, 'parameter AssetID');
				return [FALSE, ['message' => $return['message'], 'error' => $return['error']]];		 
			}	
			list($success, $return) = $this->_sql($request);
			if (!$success) return [FALSE, $return];
			$this->db->where('PortfolioID', $request->params->PortfolioID, NULL, FALSE);	
			$this->db->where('AssetID', $request->params->AssetID, NULL, FALSE);	
		}
