...
class Payablecharges_trx_model extends CI_Model
{
	...
	private function _sql($request)
	{
		...
		if (isset($request->params->fields) && !empty($request->params->fields)) 
			...
			$this->db->select('simpiID, TrxID, PortfolioID, CcyID, FeeID, TrxLinkType, TrxLinkID, 
								TrxDate, TrxDescription, TrxDebit, TrxCredit, TrxSourceID, TrxSource2ID');							
		$this->db->from('afa_payable_charges_transaction');
