...
class F {
	...
	function mail_send($email = [])
	{
		...
		if (!$email->to)
			return [FALSE, ['message' => $ci->f->lang('err_email_to')]];
		if (!$email->subject)
			return [FALSE, ['message' => $ci->f->lang('err_email_subject')]];
	...
	function mail_queue($email = [])
	{
		...
		if (!isset($email->_to)) return [FALSE, ['message' => $ci->f->lang('err_email_to')]];
	...
	function get_report($request, $data, $config)
	{
		...
		if (!isset($config['name']))
			return [FALSE, ['message' => F::lang('err_param_required', 'name')]];
		...
		if(!file_exists($reportFilePath))
			return [FALSE, ['message' => F::lang('err_report_unknown', $config['name'])]];
	...
	function batch_insert($datas)
	{
		...
		if ($ci->db->trans_status() === FALSE)
		{
			...
			return [FALSE, ['message' => F::lang('err_commit_data')]];
		}
