...
class System {
	...
	{
		$ci = $this->ci;
		$ci->db->select('a.AppsID, c.AppsCode, a.simpiID, b.LanguageID, a.emailID, d.SID, d.ClientName');
		$ci->db->from('mobc_session a');
		$ci->db->join('master_simpi b', 'a.simpiID = b.simpiID');  
		$ci->db->join('system_application c', 'a.AppsID = c.AppsID');  
		$ci->db->join('mobc_login d', 'a.simpiID = a.simpiID And a.emailID = d.emailID');  
		$ci->db->where('a.token', $request->token_id);
		$ci->db->where('a.agent', $request->agent);
		$row = $ci->db->get()->row();
		if (!$row) {
			$return = $this->error_data('00-2', $request->LanguageID, 'token_id');
			return [FALSE, ['message' => $return['message'], 'error' => $return['error']]];
		}
		$request->simpi_id = $row->simpiID;
		$request->LanguageID = $row->LanguageID;
	...
	{
		$ci = $this->ci;
		$ci->db->select('a.AppsID, a.simpiID, b.LanguageID');
		$ci->db->from('system_application a');
		$ci->db->join('master_simpi b', 'a.simpiID = b.simpiID');  
		$ci->db->where('a.AppsCode', $request->appcode);
		$row = $ci->db->get()->row();
		if (!$row) {
			$return = $this->error_data('00-2', $request->LanguageID, 'application '.$request->appcode);
			return [FALSE, ['message' => $return['message'], 'error' => $return['error']]];
		}
		$request->simpi_id = $row->simpiID;
		$request->LanguageID = $row->LanguageID;
